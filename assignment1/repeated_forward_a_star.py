...
def repeated_forward_a_star(grid_world):
    start = None
    goal = None
    ...
    while True:
        path, num_expansions = a_star_search(grid_world, start, goal)
        ...
        blocked_cells = set()
        for x, y in path:
            if grid_world.grid[y][x] == '1':
                blocked_cells.add((x, y))
        if not blocked_cells:
            return total_path, total_expansions, time.time() - start_time
        grid_world.grid = update_blocked_cells(grid_world, blocked_cells)

def update_blocked_cells(grid_world, blocked_cells):
    updated_grid = [list(row) for row in grid_world.grid]
    for x, y in blocked_cells:
        updated_grid[y][x] = '1'  # Mark blocked cells
    return updated_grid

def main_repeated_forward_a_star(filename):
    grid_world = GridWorld(filename)
    path, num_expansions, duration = repeated_forward_a_star(grid_world)
    if path:
        print("Path found:")
        for x, y in path:
            print(f"({x}, {y})")
